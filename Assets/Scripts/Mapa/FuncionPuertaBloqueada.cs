...
public class FuncionPuertaBloqueada : MonoBehaviour
{
    ...
    private FuncionBotonInteraccion scriptBoton;
    ...
    void Start() {
        ...
        scriptBoton = objetoBoton.GetComponent<FuncionBotonInteraccion>();
    ...
    void OnTriggerEnter2D(Collider2D contacto) {
        ...
        if (contacto.gameObject.CompareTag("Player") && puedeAbrirse()) {
            ...
            //Ejecuta la función del scriptBotón que permite cambiar el estado del botón interruptor
            scriptBoton.activarBoton();
